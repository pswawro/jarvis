...
export function RegionView({ period, filters, mode, onModeChange, onInteraction, onAssistantTrigger }: Props) {
  ...
  const { data } = useApi<TreeTableSpec>("/region", period, extra);
  const { data: chartData, loading: chartLoading } = useApi<LineChartSpec>("/region/chart", period, chartExtra);

  const wrapTrigger = useCallback(
    (ctx: AssistantContext) => {
      onAssistantTrigger?.({ ...ctx, view: "Region", period, filters });
    },
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-3 py-2 bg-white border-b border-gray-100">
        <h2 className="text-[13px] font-semibold text-gray-500 tracking-tight">Revenue by Region</h2>
